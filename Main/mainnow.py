...
while RUNNING:
        ...
        if 'PUMP' in msg:
            if msg.split()[2]=='HOME':
                ...
                if (int(msg.split()[2])>-1 and int(msg.split()[2])<15000):
                    if (int(msg.split()[3])>0 and int(msg.split()[3])<100):
                        if msg.split()[1] in ['SEAWATER', 'LYSIS', 'PUSH', 'PULL', 'DUMMY']: 
                            ...
                            try:
                                ...
                                param3=int(msg.split()[4])
                                
                                if msg.split()[1]=='SEAWATER':
                                    print('Activating seawater pump.')
                                    Seawater_pump.dispense(  int(msg.split()[2]) , int(msg.split()[3]), int(msg.split()[4])   )
                                if msg.split()[1]=='LYSIS':
                                    Lysis_pump.dispense(  int(msg.split()[2]) , int(msg.split()[3]), int(msg.split()[4])    )
                                if msg.split()[1]=='PUSH':
                                    Push_pump.dispense(  int(msg.split()[2]) , int(msg.split()[3]), int(msg.split()[4])    )
                                if msg.split()[1]=='PULL':
                                    Pull_pump.dispense(  int(msg.split()[2]) , int(msg.split()[3]), int(msg.split()[4])    )
                                if msg.split()[1]=='DUMMY':
                                    Dummy_pump.dispense(  int(msg.split()[2]) , int(msg.split()[3]) , int(msg.split()[4])   )
                            except:
            ...
            if not(status==MSG_GOOD):
                print('MSG is in fact BAD')
                return_msg='Error parsing PUMP command; PUMP SEAWATER 10000 1 10, for example.'
                status=MSG_BAD
        ...
        if 'COLLECT' in msg:
            if 'SEAWATER' in msg:
                ...
                Seawater_pump.dispense(15000,100, 80)
                ...
                measured_flowrate= flowrate.read()/4096*500
                ...
                pyb.delay(1000)
                feather_power.low()
                #print('Homing the seawater pump...')
                #Seawater_pump.home()
                #print('Seawater pump homed.')
            
            if status != MSG_GOOD:
                return_msg='Error parsing COLLECT command; COLLECT SEAWATER as only example.'
                status=MSG_BAD     
        ...
        if 'BACKPRESSURE' in msg:
            if 'SEAWATER' in msg:
                ...
                Seawater_pump.dispense(15000,1, 80)
            ...
            if status != MSG_GOOD:
                return_msg='Error parsing BACKPRESSURE command; BACKPRESSURE SEAWATER as only example.'
                status=MSG_BAD
